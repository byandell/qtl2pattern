...
Now partition the eigenvector matrix $U$ vertically as 
...
$$y_1 \sim N(X_1\beta,V_{11}), \quad V_{11} =\sigma^2(\gamma K_{11} + I_1)=\sigma^2(\gamma U_1DU_1^{\text{T}} + I_1)$$
...
$$CU^{\text{T}}By=CU_1^{\text{T}}y_1$$
$$CU^{\text{T}}BVBUC = CU_1^{\text{T}}V_{11}U_1C = \sigma^2CU_1^{\text{T}}U_1C^{-2}U_1^{\text{T}}U_1C=\sigma^2WW^{\text{T}}$$

with $W=CU_1^{\text{T}}U_1C^{-1}$. So we need $W^{-1} = C(U_1^{\text{T}}U_1)^{-1}C^{-1}$.

Consider a Householder decomposition of $U_1^{\text{T}}$:
$C^{-1}U_1^{\text{T}} = FG=F_1G_1$ with $G_1$ upper triangular and $FF^{\text{T}}=F^{\text{T}}F=I$,
$F=(F_1,F_2)$, $G^{\text{T}}=(G_1^{\text{T}}, 0)$. Also, $F_1^{\text{T}}F_1=I_1$.

Hence, $V_{11} = G_1^{\text{T}}F_1^{\text{T}}F_1G_1 = G_1^{\text{T}}G_1$. Thus, the model we need to solve is
...
That is, we need to take the current value of $\gamma$, do a Householder decomposition on the reduced matrix, and solve OLS.
