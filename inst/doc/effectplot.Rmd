...
(patterns <- summary(out_snppr, DOex$pmap, snpinfo))
```
...
```{r}
plot(coefs36, DOex$pmap, CC = FALSE, ylim=c(-100,100))
```
...
plot(tmp, DOex$pmap, ylim=c(-100,100))
```
...
```{r}
plot(scan_pat, DOex$pmap, "coef_and_lod", ylim=c(-100,100), xlim=c(90,110))
```
...
```{r}
alleles <- allele1(scanH = scan_apr, coefH = coefs, coefD = coefs36, 
                   scan_pat = scan_pat, map = DOex$pmap, alt = "E",
                   trim = TRUE)
```

```{r}
plot(alleles, scan_apr, DOex$pmap, frame = FALSE)
```
...
```{r}
plotly::ggplotly(plot(alleles, scan_apr, DOex$pmap, frame = TRUE), tooltip = "ids")
```
...
```{r}
plot(out_snppr, snpinfo, patterns="hilit",drop.hilit=1.5,cex=2)
```

Here is interactive version. Particularly useful when there are mutliple SV Types (`INS, DEL, InDel, ...`).

```{r}
plotly::ggplotly(plot(out_snppr, snpinfo, patterns="hilit",drop.hilit=1.5,cex=2))
```

```{r}
plot(coefs, scan1_output = scan_apr, DOex$pmap)
```

```{r}
plot(scan_pat, DOex$pmap, "coef_and_lod", ylim_coef = c(-100,100))
```
