...
```{r}
topsnps <- top_snps_pattern(out_snppr, snpinfo)
```
...
```{r}
plot_scans <- function(scans, map, lodcolumn) {
  scan_names <- colnames(scans)
  plot(scans, DOex$pmap,
     lodcolumn = lodcolumn[1], ylim = range(scans))
  legend(par("usr")[1], par("usr")[3], scan_names, col = seq_along(scan_names))
  for(i in lodcolumn[-1])
    plot(scans, DOex$pmap,
      lodcolumn = i, add = TRUE, col = i)
}
```
